...
const userId =
  process.env.DEFAULT_USER_ID ?? "11111111-1111-4111-8111-111111111111";
async function main() {
  await prisma.user.upsert({
    where: { id: userId },
    update: {},
    create: { id: userId, name: "Personal User" },
  });
  if (await prisma.debt.count({ where: { userId } })) return;
  const motor = await prisma.debt.create({
    data: {
      userId,
      name: "Cicilan Motor",
      creditor: "Leasing Contoh",
      originalPrincipal: 18000000,
      remainingPrincipal: 18000000,
      paymentPolicy: "FIXED",
  ...
  await prisma.debt.create({
    data: {
      ...
      lateFeeRule: {
        create: { calculationType: "NONE", settlementPolicy: "MANUAL" },
      },
    },
  });
  await prisma.debt.create({
    data: {
      userId,
      name: "Utang Vendor",
      creditor: "Vendor Lama",
      originalPrincipal: 7000000,
      remainingPrincipal: 7000000,
      paymentPolicy: "NEGOTIABLE",
      minimumMonthlyAmount: 0,
      targetMonthlyAmount: 500000,
      priority: "SLOW",
      canBeNegotiated: true,
      allocationPolicy: "PRINCIPAL_FIRST",
      lateFeeRule: {
        create: { calculationType: "MANUAL", settlementPolicy: "END_OF_TERM" },
      },
    },
  });
}
