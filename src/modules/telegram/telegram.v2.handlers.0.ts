...
export function registerTelegramHandlers0(bot: any) {
  ...
  bot.action(/^onboarding:country:(ID|US|SG|GB|JP|DE)$/, async (ctx) => {
    ...
    await ctx.reply("🗣 Pilih bahasa yang ingin digunakan:", {
      ...Markup.inlineKeyboard([
        [
          Markup.button.callback("🇮🇩 Bahasa Indonesia", "onboarding:language:id"),
  ...
  bot.action(/^onboarding:language:(id|en)$/, async (ctx) => {
    ...
    await ctx.reply("💱 Pilih mata uang utama untuk dashboard:", {
      ...Markup.inlineKeyboard([
        [Markup.button.callback(`⭐ ${suggested}`, `onboarding:currency:${suggested}`)],
        [
  ...
  bot.action(/^onboarding:currency:(IDR|USD|SGD|EUR|JPY|GBP)$/, async (ctx) => {
    ...
    await ctx.reply("🎨 Pilih tema bot. Tema bisa diubah lagi dari Pengaturan:", {
  ...
  bot.action(
    ...
    async (ctx) => {
      ...
      await ctx.reply("✅ Konfigurasi selesai. Semua menu sekarang siap digunakan.");
      await sendHome(ctx);
    }
  );
  ...
  bot.action(/^report:preset:(TODAY|THIS_WEEK|THIS_MONTH|PREVIOUS_MONTH|THIS_YEAR|ALL)$/, async (ctx) => {
    await ctx.answerCbQuery();
  ...
  bot.action(/^report:export:(TODAY|THIS_WEEK|THIS_MONTH|PREVIOUS_MONTH|THIS_YEAR|ALL|MONTH):(pdf|xlsx|csv):(.+)$/, async (ctx) => {
    ...
    const query = reportQuerySchema.parse({
      ...
      ...month ? { month } : {},
      grouping: preset === "THIS_YEAR" ? "MONTH" : preset === "ALL" ? "YEAR" : "NONE",
      page: 1,
      limit: 100
    });
    try {
      ...
      await ctx.reply(`❌ ${html(error?.message ?? "Gagal membuat laporan")}`, { parse_mode: "HTML" });
    }
  });
  ...
  bot.action(/^account:view:(.+)$/, async (ctx) => {
    ...
    await ctx.reply(
      `${accountIcon(account.type)} <b>${html(account.name)}</b>\n\nJenis: <b>${html(account.type)}</b>\nStatus: <b>${html(account.status ?? (account.isActive ? "ACTIVE" : "ARCHIVED"))}</b>\nMata uang: <b>${html(account.currency)}</b>\nSaldo awal: <b>${html(money(numberValue(account.openingBalance), account.currency))}</b>\nSaldo saat ini: <b>${html(money(numberValue(account.currentBalance), account.currency))}</b>`,
      {
        parse_mode: "HTML",
        ...Markup.inlineKeyboard([
          ...!account.isSystem && account.isActive ? [[Markup.button.callback("🗄 Arsipkan Account", `account:archive:start:${account.id}`)]] : [],
          ...!account.isSystem && !account.isActive ? [[Markup.button.callback("♻️ Pulihkan Account", `account:restore:${account.id}`)]] : [],
  ...
  bot.action(/^account:archive:start:(.+)$/, async (ctx) => {
    ...
    if (balance === 0) {
      sessions.set(ctx.chat.id, { kind: "ACCOUNT_ARCHIVE_CONFIRM", accountId: account.id, accountName: account.name });
      await ctx.reply(`🗄 Arsipkan <b>${html(account.name)}</b>? Histori tetap tersedia dan account dapat dipulihkan.`, {
        parse_mode: "HTML",
        ...Markup.inlineKeyboard([
    ...
    const targets = await financeService.recommendArchiveTargets(user.id, account.id);
    const sameCurrency = targets.filter((target) => target.account.currency === account.currency).slice(0, 6);
    const rows = sameCurrency.map((target) => [
      Markup.button.callback(`➡️ ${target.account.name}`, `account:archive:target:${account.id}:${target.account.id}`)
    ]);
    rows.push([Markup.button.callback("📦 Dana Belum Dialokasikan", `account:archive:unallocated:${account.id}`)]);
  ...
  bot.action(/^account:archive:target:([^:]+):([^:]+)$/, async (ctx) => {
    ...
    await ctx.reply(`Pindahkan saldo <b>${html(account.name)}</b> ke <b>${html(target.name)}</b>, lalu arsipkan account?`, {
      parse_mode: "HTML",
      ...Markup.inlineKeyboard([
        [Markup.button.callback("✅ Pindahkan & Arsipkan", "account:archive:confirm")],
        [Markup.button.callback("❌ Batal", "session:cancel")]
      ])
    });
  });
  bot.action(/^account:archive:unallocated:(.+)$/, async (ctx) => {
    ...
    await ctx.reply(`Pindahkan saldo <b>${html(account.name)}</b> ke Dana Belum Dialokasikan, lalu arsipkan account?`, {
      parse_mode: "HTML",
      ...Markup.inlineKeyboard([
        [Markup.button.callback("✅ Pindahkan & Arsipkan", "account:archive:confirm")],
        [Markup.button.callback("❌ Batal", "session:cancel")]
  ...
  bot.action("account:archive:confirm", async (ctx) => {
    ...
    try {
      await financeService.archiveAccount(user.id, state.accountId, {
        ...state.targetAccountId ? { targetAccountId: state.targetAccountId } : {},
      ...
      sessions.delete(ctx.chat.id);
      await ctx.reply(`✅ Account <b>${html(state.accountName)}</b> berhasil diarsipkan. Histori tetap tersedia.`, { parse_mode: "HTML", ...backHome() });
    } catch (error) {
      await ctx.reply(`❌ ${html(error?.message ?? "Gagal mengarsipkan account")}`, { parse_mode: "HTML" });
    }
  });
}
