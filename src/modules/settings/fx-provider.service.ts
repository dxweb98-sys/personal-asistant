...
async function currenciesInUse(userId: string): Promise<string[]> {
  const [accounts, debts, instruments, transactions, dividends] = await Promise.all([
    prisma.financialAccount.findMany({ where: { userId, isActive: true }, select: { currency: true } }),
    prisma.debt.findMany({ where: { userId }, select: { currency: true } }),
    prisma.investmentInstrument.findMany({ where: { userId }, select: { currency: true } }),
    prisma.financialTransaction.findMany({ where: { userId }, distinct: ["currency"], select: { currency: true } }),
    prisma.dividendIncome.findMany({ where: { userId }, distinct: ["currency"], select: { currency: true } }),
  ]);

  return [...new Set([
    ...accounts.map((x: any) => x.currency),
    ...debts.map((x: any) => x.currency),
    ...instruments.map((x: any) => x.currency),
    ...transactions.map((x: any) => x.currency),
    ...dividends.map((x: any) => x.currency),
  ].map(upper).filter(Boolean))].sort();
}
...
async function fetchReferenceRate(fromCurrency: string, toCurrency: string) {
  ...
  try {
    const response = await fetch(url, {
      headers: { accept: "application/json", "user-agent": "personal-finance-os/9" },
      signal: controller.signal,
    });
    if (!response.ok) throw new Error(`Provider merespons HTTP ${response.status}`);
    const payload = await response.json() as { date?: string; rates?: Record<string, number> };
    const rate = Number(payload.rates?.[to]);
    if (!(rate > 0)) throw new Error(`Kurs ${from}/${to} tidak tersedia dari provider`);
    const capturedAt = payload.date ? new Date(`${payload.date}T16:00:00.000Z`) : new Date();
...
export const fxProviderService = {
  ...
  async refreshUsedCurrencies(userId: string, baseCurrency: string): Promise<FxRefreshItem[]> {
    const target = upper(baseCurrency);
    ...
    for (const pair of preview) {
      ...
      if (!pair.supported) {
        results.push({ fromCurrency: from, toCurrency: target, status: "SKIPPED", reason: "Bukan kode fiat ISO 4217; masukkan kurs manual atau gunakan provider crypto." });
        continue;
      }
      ...
      try {
        ...
        await upsertExchangeRate(userId, from, target, latest.rate, "API", latest.capturedAt);
        ...
        results.push({
          ...
          toCurrency: target,
          status: "FAILED",
          reason: error instanceof Error ? error.message : "Gagal mengambil kurs",
        });
      }
    }
    return results;
