...
export async function buildRecommendation(
  ...
  let allocated = allocations.reduce((a, x) => a + x.amount, 0);
  ...
  if (allocated > available) {
    ...
    allocated = allocations.reduce((a, x) => a + x.amount, 0);
    if (allocated > available) {
      const ratio = available / allocated;
      allocations.forEach((x) => (x.amount = roundMoney(x.amount * ratio)));
      allocated = available;
    }
  }
  ...
  for (const d of sort(debts, input.strategy)) {
    if (extra <= 0) break;
    const x = allocations.find((a) => a.debt.id === d.id)!;
    const target =
